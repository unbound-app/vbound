...
Both build steps redirect `2>&1` into the captured pipe and route every line through `appendBuildLog`. This matters more than it looks: `ubd build` prints its per-addon progress to stdout but reports *which* addon failed (and why — usually a workspace whose `bun install` never ran, so `rollup` isn't on its PATH) only on stderr, and it exits non-zero well after the last stdout line looked like a success. Without the redirect the whole failure was invisible and surfaced as a bare "Addon build failed"; `looksLikeBuildFailure`/`buildFailureMessage` now pull the explanatory line into the toast. Relatedly, `run(args:onOutput:)` delivers *every* line (batched per read, via `ProcessLineBuffer` so a line split across a read boundary isn't mangled) rather than only the last line of each chunk — the old behavior silently dropped most of the build log and made the tweak's progress estimate undercount, since step markers that weren't at the end of a chunk were never counted.

The `.deb` upload goes through `uploadDeb(from:to:)`, not a bare `run(args:)` `scp` — it primes the SSH mux via `primeSSHControlMaster()` first and retries up to 3 times (closing the mux master and re-running `ensurePortForward()` between attempts), for the same reason `deployPlugins` primes: `ensurePortForward()`'s `nc -z` readiness probe can pass before the usbmux tunnel can actually carry an SSH handshake, and a single-shot `scp` landing in that window failed outright with a bare "Upload failed". It runs through `runCapturingOutput` (stdout+stderr merged) so the real `scp`/`ssh` error lands in the build log and in the failure toast (`Upload failed — <last line>`) instead of being written to the app's own stderr where nobody sees it.

### SSH / port forwarding

Everything SSH-related targets `mobile@127.0.0.1:2222` (`pymobiledevice3 usbmux forward 2222 22`, brought up on demand by `ensurePortForward()`), authenticated with `sshpass` using the password from Settings (`AppStorage("sshPassword")`, defaults to vphone's stock `alpine`). An SSH `ControlMaster`/`ControlPath` mux (`AppController.sshControlPath`, under `~/.ssh/vbound-mux`) is shared across build/shell/shutdown so repeated commands don't each pay the SSH handshake cost. That master outlives the tunnel it was opened over (`ControlPersist=60`), so whenever `ensurePortForward()` decides to *replace* a dead forward it calls `closeSSHControlMaster()` (`ssh -O exit`) first — otherwise every subsequent `scp`/`ssh` multiplexes onto a socket whose underlying connection is already gone and fails instead of dialing a fresh one.
